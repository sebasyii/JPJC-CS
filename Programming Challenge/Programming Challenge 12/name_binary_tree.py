...
class Tree:
    ...
    def add(self, newItem):
        ...
        new_next_free = self.__tree[self.__nextFreePosition].get_left_child() # Get the next pointer
    ...
    def __str__(self): #print method to display tree on screen
        ...
        output = output + "{:^5}{:^10}{:^5}{:^5}\n".format("Index","Data","LeftPtr","RightPtr")
        for i in range(1,self.__limit):
            output = output + "{:^5}{:^10}{:^5}{:^5}\n".format(i, self.__tree[i].get_data(), self.__tree[i].get_left_child(),self.__tree[i].get_right_child())
        return output
