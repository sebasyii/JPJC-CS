...
class Tree:
    ...
    def add(self, newItem):
        ...
        self.__tree[self.__nextFreePosition].set_left_child(
            0)  # Set it to last node
    ...
    def __str__(self):  # print method to display tree on screen
        ...
        for i in range(1, self.__limit):
            output = output + "{:^5}{:^10}{:^5}{:^5}\n".format(i, self.__tree[i].get_data(
            ), self.__tree[i].get_left_child(), self.__tree[i].get_right_child())
        return output
